...
#endif
#if defined(CONFIG_MACH_CRATERTD_CHN_3G) || defined(CONFIG_MACH_CRATER_CHN_CTC)
...
//MSM_GPIO_TO_INT(GPIO_TOUCH_IRQ);
...
static void msm_tkey_led_vdd_on(bool onoff)
{
	...
	if (!reg_l36) {
		...
		reg_l36 = regulator_get(NULL, "8917_l33");
#endif
		if (IS_ERR(reg_l36)) {
	...
	enum led_brightness value)
{
	...
	struct zinitix_touch_dev *info =
		container_of(led_cdev, struct zinitix_touch_dev, leds);
...
static void ts_power_onoff(int onoff)
{
	int ret = 0;
...
static void zinitix_parsing_data(struct zinitix_touch_dev *touch_dev)
{
	...
	if (touch_dev->touch_mode != TOUCH_POINT_MODE && touch_dev->sec_mode)
		goto continue_parsing_data;
...
#if (TOUCH_POINT_MODE == 2)
			//printk(KERN_INFO "[TSP] w: %d, minor_width: %d, angle: %d, palm: %d \n", w, 
			//	(u32)touch_dev->touch_info.coord[i].minor_width, touch_dev->touch_info.coord[i].angle, palm);
			input_report_abs(touch_dev->input_dev,
...
#define ZINTIX_DEVICE_NAME "SCH-P709"
#define FW_RELEASE_DATE "0507"
#else
#define ZINTIX_DEVICE_NAME "GT-I9158"
#define FW_RELEASE_DATE "0507"
#endif
...
static void get_config_ver(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%s_%s_%s",
	    ZINTIX_DEVICE_NAME, CHIP_INFO, FW_RELEASE_DATE);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));
...
static int set_conifg_flip_cover(int enables)
{
    ...
    printk("[TSP] set_conifg_flip_cover : current_cable_type = %d !!\n",current_cable_type);

    if(current_cable_type ==11)
    {
...
//	}
	...
	if (!gpio_get_value(misc_touch_dev->int_gpio_num)) {
		zinitix_printk("interrupt occured during btn set+\r\n");
		zinitix_parsing_data(misc_touch_dev);
	}
	enable_irq(misc_touch_dev->irq);
	return 0;
}
...
{
	struct zinitix_touch_dev *info = misc_touch_dev;
		...
		do {
			if (*cur == delim || cur - buf == len) {
				...
				*(buff + strlen(buff)) = '\0';
				...
				if(ret < 0)
					printk(KERN_ERR "%s:str to int error!ret=%d,buff=%s",
						__func__,ret,buff);
				start = cur + 1;
				memset(buff, 0x00, ARRAY_SIZE(buff));
				param_cnt++;
...
//	ret = request_threaded_irq(touch_dev->irq, NULL, zinitix_touch_work,	IRQF_TRIGGER_LOW , ZINITIX_DRIVER_NAME, touch_dev);
#else		
...
//	ret = request_threaded_irq(touch_dev->irq, NULL, ts_int_handler,	IRQF_TRIGGER_LOW | IRQF_ONESHOT, ZINITIX_DRIVER_NAME, touch_dev);
#endif
...
#if defined(CONFIG_MACH_CRATERTD_CHN_3G) || defined(CONFIG_MACH_CRATER_CHN_CTC)
#ifdef CONFIG_LEDS_CLASS
	touch_dev->leds.name = TOUCHKEY_BACKLIGHT;
	touch_dev->leds.brightness = LED_FULL;
	touch_dev->leds.max_brightness = LED_FULL;
	touch_dev->leds.brightness_set = msm_tkey_led_set;

	ret = led_classdev_register(&client->dev, &touch_dev->leds);
	if (ret) {
		dev_err(&client->dev,
...
#ifdef CONFIG_LEDS_CLASS
fail_led_reg:
	led_classdev_unregister(&touch_dev->leds);
#endif
#endif
err_request_irq:
